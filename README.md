...
[![版本](https://img.shields.io/badge/version-0.2.2-0F766E.svg)](.codex-plugin/plugin.json)
...
| `api-blackbox-test-planner` | 输出编码前或执行前的接口黑盒测试方案。 |
| `api-blackbox-test-executor` | 执行真实接口请求，使用 `db-mcp` 准备或校验数据，并记录证据。 |
| `api-blackbox-test-reporter` | 汇总方案和执行证据，输出覆盖率、风险和发布建议。 |

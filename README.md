...
接口变更经常能通过单元测试，却仍然破坏客户端契约、数据库副作用或多接口业务链路。`api-blackbox-tester` 为 Codex 提供一套可重复的接口黑盒测试流程，把测试方案、用例评审、执行证据和最终报告串联起来。
...
| Skill | 作用 |
| --- | --- |
| `api-blackbox-tester` | 统一入口，根据用户意图选择并串联四个测试阶段。 |
...
2. 使用 `api-blackbox-test-reviewer` 组织四角色评审测试用例、数据计划、边界测试、安全测试和接口编排链路。

...
根据 planner 阶段输出的测试用例、断言计划、证据采集计划、测试数据计划、接口 x 字段语义等价类矩阵、跨接口一致性用例和接口编排/集成链路，以及 reviewer 阶段通过或有条件通过的多角色评审结论，执行真实接口请求并产出可供 reporter 汇总的证据包。
...
- 如果上下文中没有影响点与影响范围评估，必须先基于需求分析和技术方案判断测试范围是否充分，再优先选择当前项目可用的结构化路由分析工具识别接口、路由、消费者、任务和编排链路；没有可用工具时，再用代码搜索、代码 diff、API 文档、路由表和调用链分析识别公共方法、共享组件、关联接口、调用方、数据模型、副作用、缓存/异步、安全边界、兼容性和观测影响。
...
- 所有核心用例必须记录断言执行记录和证据包。用例通过必须基于断言通过、响应证据和必要 DB/副作用证据，不能只依据 HTTP 200 或接口未报错。

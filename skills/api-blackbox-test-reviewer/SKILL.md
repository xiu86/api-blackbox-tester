...
description: 测试用例质量门禁 Skill。当 planner 输出测试方案后、executor 执行前，组织资深测试工程师、资深产品经理、资深软件工程师、安全工程师、测试证据审计员五个角色进行风险分级评审、业务场景拆解评审、覆盖矩阵校验、接口 x 字段语义等价类矩阵逐单元评审、场景 x 边界/异常覆盖矩阵评审、跨接口一致性评审、可执行性 preflight、需求追踪、安全建模、证据完整性评审和 Anti-Happy-Path 检测，并给出通过、有条件通过、不通过或阻塞的最终结论。
---
...
- 需求分析、技术方案、结构化路由影响分析结果、代码 diff、相关接口/模块路径或调用链；缺少代码或调用关系证据时，先基于需求分析和技术方案判断测试范围是否充分，再通过当前项目可用的结构化路由分析工具、代码搜索或项目文档补充受影响逻辑、公共方法和关联接口分析。
- planner 输出的 `结构化路由影响分析` 和 `变更代码影响接口清单`。如果缺失该清单，reviewer 必须自行优先选择当前项目可用的结构化路由分析工具补做影响接口映射；没有可用工具时，再基于代码 diff、路由表、调用链、代码搜索、API 文档或项目约定补做。无法补做时应判定为 `阻塞` 或 `不通过`，不得直接进入 executor。
...
- **测试证据审计员**：检查断言计划、证据采集计划、证据包可生成性、DB/副作用证据、阻塞补救证据、复验证据和报告可追溯性；核心用例缺少证据路径、写接口缺少 DB/副作用证据、跨接口一致性缺少每个参与接口的独立证据时，不得给出 `通过`。
...
- 是否在 `场景 x 边界/异常覆盖矩阵` 中关联正常、异常、边界和权限/状态/数据变体用例。
...
资深软件工程师必须先基于需求分析和技术方案判断测试范围是否充分，再优先结合当前项目可用的结构化路由分析工具结果，必要时补充代码 diff、API 文档、调用链、配置、数据库 schema、消息/异步链路和前后端消费者分析来追踪扩散影响。评估不能只看本次显式改动接口；如果改动公共方法、共享组件、公共校验、公共查询、缓存逻辑、权限逻辑、序列化结构、错误处理或数据模型，必须识别所有可能受影响的接口、任务、消费者和业务链路，并纳入测试验证范围。

评估必须覆盖：

- 本次代码变更文件/符号到接口的映射关系，包括直接变更接口、间接受影响接口、逻辑类似接口、前后置接口、回调接口、消息入口和后台任务触发接口。
...
如果上下文没有足够信息，必须先根据需求分析和技术方案给出初步影响范围，再优先选择当前项目可用的结构化路由分析工具补充分析；没有可用工具时，再使用代码搜索、路由定义、API 文档或项目文档补充分析。无法分析时应标记阻塞或把缺口写入退回项。所有直接影响和扩散影响必须映射到测试用例、矩阵单元、跨接口一致性用例或编排链路；未覆盖的影响不得静默放过，必须补充用例或给出明确豁免依据。

凡是影响范围关联到接口或接口链路的对象，必须确认该接口已经进入 planner 的 `测试范围`、`接口 x 字段语义等价类矩阵`、`跨接口一致性测试` 或 `接口编排/集成测试`。仅说明“代码未直接改该接口”不能作为排除依据；只有存在明确业务差异、调用路径不可达、权限隔离、历史契约或技术证据时，才可豁免。
...
- 业务场景层面也必须检测 Happy Path 偏置：任一核心业务场景只有正常路径，没有适用边界值、异常路径、权限/状态/数据变体或跨接口失败链路时，应标记为 `业务场景 Happy Path 偏置`。
...
- 辩论不替代五角色多角色评审和硬性门禁；它是发现遗漏的附加质量门禁。
...
3. **独立评审**：五个角色分别列出通过项、问题项、建议项和阻塞项，并完成各自强制职责。

...
description: 接口黑盒测试报告 Skill。当用户需要根据编码前测试方案、测试用例评审结论和编码后真实执行证据，评估需求覆盖率、用例执行覆盖率、接口覆盖率、边界测试覆盖率、安全测试覆盖率、编排/集成链路覆盖率、性能测试建议、兼容性结论、测试是否通过，并为未通过、部分通过、阻塞和剩余风险输出根因分析、解决建议、复验范围和测试报告时使用。
---
...
基于 planner 的测试方案、reviewer 的测试用例评审结论和 executor 的真实执行证据，输出最终测试报告。没有真实请求和 DB 验证证据时，不得给出“通过”结论，只能说明证据不足或阻塞。

如果测试报告结论不是 `通过`，本阶段必须输出可回到开发阶段执行的修复输入，并要求修复后重新进入 `executor 复验 -> reporter 再评估`。只有新的测试报告结论为 `通过` 时，完整黑盒测试流程才算闭环完成。
...
- 不得给出发布通过建议。
- 必须输出“开发修复与复验计划”。
- 必须说明修复后需要回到 executor 执行真实 HTTP/gRPC 复验，再由 reporter 基于复验证据重新评估。
